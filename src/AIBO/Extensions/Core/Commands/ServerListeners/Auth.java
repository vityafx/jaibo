...
public final class Auth extends SimpleCommand implements ConfigurationListener {
    ...
    private void readAuthenticationSettings() {
        String authenticationNeed = AIBO.Configuration.getConfigurationHashMap().get("Authentication.authentication");

        if (authenticationNeed != null && authenticationNeed.equalsIgnoreCase("yes")) {
            this.authenticationNeed = true;

            this.username = AIBO.Configuration.getConfigurationHashMap().get("Authentication.username");
            this.password = AIBO.Configuration.getConfigurationHashMap().get("Authentication.password");
        }
    }
